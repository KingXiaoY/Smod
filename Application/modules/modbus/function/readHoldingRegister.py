...
class Module:
	...
	def do(self, ip, StartAddr, Quantity):
		...
		ans = c.sr1(ModbusADU(transId=getTransId(),unitId=int(self.options['UID'][0]))/ModbusPDU03_Read_Holding_Registers(startAddr=int(StartAddr,16),quantity=int(Quantity,16)),timeout=timeout, verbose=0)
	...
	def do_copy(self, ip, StartAddr, Quantity):
		...
		if(c == None):
			...
			return None
		self.printLine('[+] Connecting to ' + ip,bcolors.OKGREEN)
		ans = c.sr1(ModbusADU(transId=getTransId(),unitId=int(self.options['UID'][0]))/ModbusPDU03_Read_Holding_Registers_origin(startAddr=int(StartAddr,16),quantity=int(Quantity,16)),timeout=timeout, verbose=0)
		ans = ModbusADU_Answer(str(ans))
		self.printLine('[+] Response is :',bcolors.OKGREEN)
		ans.show()
		time.sleep(5)
